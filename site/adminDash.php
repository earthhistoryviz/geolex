...
<div class="container" style="height: 100%;">
    ...
    <div class="left-menu"> <?php
        if ($timescaleExists) { ?>
          <div class="item <?php if ($_SERVER["PHP_SELF"] == "/adminIndex.php" && empty($_SERVER["QUERY_STRING"])) {
              echo "active";
          } ?>" >
          ...
          <div class="item <?php if (preg_match("/fileBrowser.php/", $_SERVER["PHP_SELF"]) || preg_match("/Upload.php/", $_SERVER["PHP_SELF"])) {
              echo "active";
          } ?>" >
            <a class="menu-link" href="/fileBrowser.php">Parse Word Document</a>
          </div>
          <div class="item <?php if (preg_match("/mapPackBrowser.php/", $_SERVER["PHP_SELF"])) {
              echo "active";
          } ?>" >
            <a class="menu-link" href="/mapPackBrowser.php">Parse TSC MapPack Docs </a>
          </div>
          <div class="item <?php if (preg_match("/lithologyPatternCheckBrowser.php/", $_SERVER["PHP_SELF"])) {
              echo "active";
          } ?>" >
            <a class="menu-link" href="/lithologyPatternCheckBrowser.php">Check Lithology Pattern </a>
          </div>
          <div class="item <?php if (preg_match("/uploadTimescale.php/", $_SERVER["PHP_SELF"])) {
              echo "active";
          } ?>" >
            <a class="menu-link" href="/uploadTimescale.php">Timescale</a>
          </div>
          <div class="item <?php if (preg_match("/manageUser.php/", $_SERVER["PHP_SELF"]) || preg_match("/Signup.php/", $_SERVER["PHP_SELF"])) {
              echo "active";
          } ?>" >
          ...
          All other admin features disabled until a default timescale is set up <?php
        } ?>
