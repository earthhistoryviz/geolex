    ...
    foreach ($splitcontent as $ministr) {
        ...
        if($vars[$geojsonindex]["value"] == "null" && !empty($origgeoJSON)) {
            if (preg_match("/Fossils:/", $origgeoJSON)) {
                ...
                echo "Error: Invalid geoJSON data. Please recheck for misplaced punctuation or brackets. The invalid geoJSON that was in the document was: <pre>";
                htmlspecialchars(print_r($origgeoJSON));
                echo "</pre>";
            }
        ...
        if ($conn->query($sql) === true) {
            ...
            echo "The array of extractions which produced that query was: <pre>";
            print_r($vars);
            echo "</pre>";
            echo "-------------------------------------------------\n\n<br><br>";
        }
